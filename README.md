...
- **10 tools**: `read_file`, `write_file`*, `edit_file`*, `list_dir`, `glob`, `grep`, `run_command`*, `todo_write`, `fetch_url`, `task`* — the starred ones show a y/n approval card before executing. `task` spawns a one-level sub-agent with a fresh conversation whose tool calls run without further prompts.
