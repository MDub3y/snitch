...
`<Static>` transcript (past messages + resolved tool cards, flicker-free) → live streaming tail → active ToolCallCard `[y]/[n]` → InputBox → StatusBar (model, spinner, token/cost tally). One app-level mode (`input | working | approval`) decides whose `useInput` is live; Esc triggers an `AbortController`. The input line stays live while the agent works: Enter queues the message (shown as a dim `[queued]` line) and a drain effect submits queued items one at a time when the loop goes idle, so a queued `/model` switch applies to tasks queued behind it. Input deactivates only during approval, so `y`/`n` cannot leak into the next task. Slash commands: `/help`, `/clear` (new History + `<Static>` remount via a generation key), `/plan` (toggle plan mode: the run uses `registry.readOnlyView()` — approval-requiring tools simply do not exist for the model — plus a plan-only instruction; cyan `[plan]` badge in the status bar), `/compact` (model-written summary replaces everything but the recent tail; also runs automatically before a task when the history estimate exceeds 80% of the token budget), `/model <id>` (rebuilds the provider), `/exit`.

Compaction vs trimming: `toMessages()` trimming silently drops old messages at send time as a safety net; compaction (`agent/compact.ts` + `History.compact()`) preserves the dropped content in condensed form and actually shrinks the stored history. The compacted tail never starts on an orphaned tool result. Standard terminal exits are honored too: bare `exit`/`quit`/`q`/`:q` at the prompt and Ctrl+D (EOF) quit instead of being sent to the model.

### CLI

`snitch [--headless "<prompt>"] [--yes] [--plan] [--model <id>] [--prompt-tools]` — headless mode drives the same agent loop with readline y/n approvals (stdout: reply text; stderr: tool/status lines). `--yes`/`-y` auto-approves every tool call for scripted/piped runs; if stdin has ended when an approval is due, the call is denied rather than crashing.
